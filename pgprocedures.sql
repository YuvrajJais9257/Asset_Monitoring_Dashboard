CREATE OR REPLACE PROCEDURE public.update_report_name(
    ...
    IN old_chart_type character varying,
    IN new_chart_type character varying
)
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    rec RECORD;
BEGIN
    -- Loop over only those rows where any array element matches old_report_name, old_report_type, or old_chart_type
    FOR rec IN
        ...
        WHERE EXISTS (
            ...
            WHERE elem ->> 'chartType' = old_report_name
               ...
               OR elem ->> 'chart_type' = old_chart_type
        )
    LOOP
        ...
        FOR i IN 0 .. jsonb_array_length(report_data) - 1 LOOP
            ...
            IF (report_data -> i ->> 'chart_type') = old_chart_type THEN
                updated_report_data := jsonb_set(
                    updated_report_data,
                    ARRAY[i::text, 'chart_type'],
                    to_jsonb(new_chart_type)
                );
            END IF;
        END LOOP;
