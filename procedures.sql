...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateReportName`(
    ...
    IN old_chart_type VARCHAR(255),
    IN new_chart_type VARCHAR(255)
)
BEGIN
    ...
    DECLARE cur CURSOR FOR
        ...
        WHERE JSON_CONTAINS(dashboard_json_frame_data, JSON_OBJECT('chartType', old_report_name))
           ...
           OR JSON_CONTAINS(dashboard_json_frame_data, JSON_OBJECT('chart_type', old_chart_type));
    ...
    read_loop: LOOP
        ...
        WHILE report_data IS NOT NULL AND @i < JSON_LENGTH(report_data) DO
            ...
            -- Update chart_type if it matches old_chart_type
            IF JSON_UNQUOTE(JSON_EXTRACT(report_data, CONCAT('$[', @i, '].chart_type'))) = old_chart_type THEN
                SET updated_report_data = JSON_SET(updated_report_data, CONCAT('$[', @i, '].chart_type'), new_chart_type);
            END IF;
