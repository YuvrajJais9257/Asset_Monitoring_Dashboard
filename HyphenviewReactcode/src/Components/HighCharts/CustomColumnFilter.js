...
import { getfilterdatadrillval, getdataforDrilldownval } from "../../actions/reportmanagement";
...
const CustomColumnFilter = ({ setOpenCustomFilter, columns, data, pagination, setShowcolumnFilter, conditions, setConditions, sorting, length }) => {
    ...
    const handleSelectedData = async (index, value) => {
        ...
        const formData = {
            ...
            filter_value: data?.category_value,
            selectedSeriesName: data?.selected_series_name,
            flag: "drilldownpage"
        }
    ...
    const handleAddApplyFilter = async () => {
        ...
        try {
            if (data?.category_value && data?.report_title && data?.selected_series_name) {
                await dispatch(getfilterdatadrillval
                    ({
                        report_name: data?.report_title,
    ...
    const handleClearFilter = async () => {
        try {
            ...
            setLoading(true);
            if (data?.report_title && data?.category_value) {
                await dispatch(
                    getdataforDrilldownval({
                        customer_id: user?.customer_id,
                        master_report: data?.report_title,
                        filter_value: data?.category_value,
                        selectedSeriesName: data?.selected_series_name,
                        page_no: pagination.pageIndex + 1,
                        page_size: pagination.pageSize,
