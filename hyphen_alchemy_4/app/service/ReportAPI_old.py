...
from service.GeneratePDFandExcel import generate_excel_report_2, generate_pdf_report_2, generate_csv_report
...
class ReportManager:
    ...
    def __init__(self):
        ...
        #     "host": self.conf["oracle"]["oracle_host"],
        #     "port": self.conf["oracle"]["oracle_port"],
        #     "username": self.conf["oracle"]["oracle_username"],
        #     "password": self.conf["oracle"]["oracle_password"],
        #     "database": self.conf["oracle"]["oracle_new_schema"],
        # }
    ...
    async def get_report_detail(self, user_details: dict):
        try:
            ...
            if not res:
                ...
                raise HTTPException(
                    status_code=404, detail="Database details not found"
                )
    ...
    async def get_report_data_id(self, report_details: dict):
        ...
        # with database_mysql.cursor(dictionary=True) as cursor:
            ...
            if report_type.lower() == "chart":
                ...
                if type(result[0]) == dict or type(result[0]) == psycopg2.extras.RealDictRow or type(result[0]) == RealDictRow:
                    report_value = result[0][report_key]
                else:
                    report_value = result[0][0]
    ...
    async def getdata(self, details: dict):
        ...
        try:
            ...
            if not result:
                ...
                raise HTTPException(
                    status_code=404, detail="Detailed report not found"
                )
            ...
            if not result:
                ...
                raise HTTPException(
                    status_code=404, detail="Report template not found"
                )
    ...
    async def get_bulk_chart(self, user_details: dict):
        ...
        try:
            ...
            try:
                ...
                logging.error(f"Database connection failed: {db_connect_error}", exc_info=True)
                return JSONResponse(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            if flag == "drilldownpage":
                try:
                    ...
                    return JSONResponse(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                        content=f"Error fetching master report: {fetch_template_error}",
                    )
    ...
    async def generate_reports(self, user_details: dict):
        ...
        try:
            ...
            try:
                ...
                return JSONResponse(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    content=f"Error creating report: {e}",
                )
